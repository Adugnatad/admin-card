...
const CardTable = () => {
  ...
  useEffect(() => {
    setIsMounted(true);
    const storedTab = localStorage.getItem("selectedTab") as
      | "individual"
      | "group";
    if (storedTab) {
  ...
  const totalPages = Math.max(
    ...
    Math.ceil(
      (selectedTab === "individual"
        ...
        : filteredGroupData.length) / itemsPerPage
    )
  );
  ...
  return (
    <div className="grid grid-cols-12 gap-4 p-4">
                ...
                >
                  ...
                  {card.status !== "pending" &&
                    ...
                    card.status !== "printed" && (
                      <TableCell>
                        <button
                          className="bg-[#00adef] text-white px-3 py-1 rounded hover:bg-[#00adef] transition duration-300"
                          onClick={() => handleModal("print", card)}
                        >
                          Print
                        </button>
                        <button
                          className="bg-red-700 ml-4  text-white px-3 py-1 rounded hover:bg-red-700 transition duration-300"
                          onClick={() => handleModal("reject", card)}
                        >
                          Reject
                        </button>
                      </TableCell>
              ...
              {paginatedGroupData.map((group) =>
                group.confirmed_cards.map((member, index) => (
                  ...
                  >
                    {index === 0 && (
                      <>
                        <TableCell rowSpan={group.confirmed_cards.length}>
                          {group.creator}
                        </TableCell>
                      </>
                    )}
                    ...
                    <TableCell>
                      <button
                        className="bg-[#00adef] text-white px-3 py-1 rounded hover:bg-[#00adef] transition duration-300"
                        onClick={() => handleModal("print", group)}
                      >
                        Print
                      </button>
                      <button
                        className="bg-red-700 ml-4  text-white px-3 py-1 rounded hover:bg-red-700 transition duration-300"
                        onClick={() => handleModal("reject", group)}
                      >
                        Reject
                      </button>
                    </TableCell>
      ...
      <div
        className="col-span-4 flex items-center justify-center bg-gray-100 p-4 rounded-lg shadow-md"
        style={{ height: "250px" }}
      >
        {selectedCard && (
          <img
      ...
      {modalData.isOpen && (
        <Dialog open={modalData.isOpen} onOpenChange={closeModal}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {modalData.action === "print" ? "Print Card" : "Reject Card"}
              </DialogTitle>
              <DialogDescription>
                ...
                {modalData.customer
                  ? "card_id" in modalData.customer
                    ...
                    : // For group cards, find the matching card using selectedCardId
                      modalData.customer.confirmed_cards.find(
                        (member: any) => member.card_id === selectedCardId
                      )?.user_name || "unknown"
                  : "unknown"}
                ?
              </DialogDescription>
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              <Button
                onClick={async () => {
                  try {
                    const status =
                      modalData.action === "print" ? "printed" : "rejected";
                    const payload: {
                      id: string;
                      status: string;
                      reason?: string;
                    ...
                    await api.put(
                      `api/v1/cards/confirmed-cards-update/${payload.id}`,
                      payload
                    );
                    closeModal();
